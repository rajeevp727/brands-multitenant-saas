...
const RestaurantMenu = ({ restaurant, isOpen, onClose }: RestaurantMenuProps) => {
  ...
  useEffect(() => {
    const fetchMenuItems = async () => {
      ...
      setIsLoading(true)
      setError(null)
      setVisibleCount(6) // Reset pagination when opening a new menu
  ...
  const handleScroll = (e: React.UIEvent<HTMLDivElement>) => {
    ...
    if (isAtBottom && !isPaginating && visibleCount < menuItems.length) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow-2xl w-full max-w-4xl flex flex-col" style={{ maxHeight: '90vh' }}>
        {/* Header */}
            ...
            <div className="flex flex-col">
              ...
              {isPaginating && (
                <div className="flex justify-center items-center py-6">
                  ...
                  <span className="ml-2 text-gray-500 text-sm">Loading more items...</span>
                </div>
              )}
            </div>
          )}
