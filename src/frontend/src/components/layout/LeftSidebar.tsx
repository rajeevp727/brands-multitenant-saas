...
export default function LeftSidebar({ open, collapsed, onClose, onToggleCollapse, variant = 'permanent' }: { open: boolean; collapsed: boolean; onClose: () => void; onToggleCollapse: () => void; variant?: 'permanent' | 'persistent' | 'temporary'; }) {
  ...
  return (
    <Drawer variant={variant} open={open} onClose={onClose} PaperProps={{ sx: { pointerEvents: 'auto' } }} sx={{ width: currentWidth, flexShrink: 0, whiteSpace: 'nowrap', '& .MuiDrawer-paper': { width: currentWidth, boxSizing: 'border-box', backgroundColor: mode === 'dark' ? alpha(theme.palette.background.default, .9) : alpha('#ffffff', .8), backdropFilter: 'blur(16px)', borderRight: `1px solid ${theme.palette.divider}`, overflowX: 'hidden' } }}>
      <Box sx={{ p: 2, display: 'flex', alignItems: 'center', justifyContent: collapsed ? 'center' : 'space-between', borderBottom: `1px solid ${alpha(theme.palette.divider, .5)}`, minHeight: 64, mb: 1 }}>
        {!collapsed && user && <Box sx={{ overflow: 'hidden', mr: 1 }}><div style={{ fontWeight: 700, fontSize: '.95rem' }}>{user.username || user.email?.split('@')[0]}</div><div style={{ fontSize: '.75rem', color: String(theme.palette.text.secondary) }}>{user.email}</div></Box>}
      ...
      <List sx={{ px: 1.5, mt: 'auto', mb: 2 }}>
        {!user ? <ListItem disablePadding sx={{ display: 'block' }}><Tooltip title={collapsed ? 'Join / Sign In' : ''} placement="right"><ListItemButton onClick={(e) => { e.stopPropagation(); nav('/login'); }} sx={{ minHeight: 48, justifyContent: collapsed ? 'center' : 'initial', px: 2.5, borderRadius: 3, bgcolor: alpha(theme.palette.primary.main, .1), color: theme.palette.primary.main }}><ListItemIcon sx={{ minWidth: 0, mr: collapsed && variant !== 'temporary' ? 'auto' : 2, justifyContent: 'center', color: 'inherit' }}><LoginIcon /></ListItemIcon><ListItemText primary="Join / Sign In" sx={{ opacity: collapsed && variant !== 'temporary' ? 0 : 1 }} /></ListItemButton></Tooltip></ListItem> : <ListItem disablePadding sx={{ display: 'block' }}><Tooltip title={collapsed ? 'Logout' : ''} placement="right"><ListItemButton onClick={(e) => { e.stopPropagation(); logout(); navigate('/login'); }} sx={{ minHeight: 48, justifyContent: collapsed ? 'center' : 'initial', px: 2.5, borderRadius: 3, color: theme.palette.error.main }}><ListItemIcon sx={{ minWidth: 0, mr: collapsed && variant !== 'temporary' ? 'auto' : 2, justifyContent: 'center', color: 'inherit' }}><LogoutIcon /></ListItemIcon><ListItemText primary="Logout" sx={{ opacity: collapsed && variant !== 'temporary' ? 0 : 1 }} /></ListItemButton></Tooltip></ListItem>}
